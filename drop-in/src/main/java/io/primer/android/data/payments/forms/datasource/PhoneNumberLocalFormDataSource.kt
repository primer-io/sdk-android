...
internal class PhoneNumberLocalFormDataSource(
    ...
    private val logos = mapOf(
        PaymentMethodType.ADYEN_MBWAY to mapOf(
        ...
        PaymentMethodType.XENDIT_OVO to mapOf(
            true to R.drawable.ic_logo_mbway_dark,
            false to R.drawable.ic_logo_mbway_light
        )
    )
            ...
            FormDataResponse(
                ...
                inputs = listOf(
                    FormInputDataResponse(
                        ...
                        validation = null, // Validation is handled in the component
                        inputPrefix = DialCodeCountryPrefix(
    ...
    private fun getLogo(isDarkMode: Boolean) =
        when (paymentMethodType) {
            ...
            PaymentMethodType.XENDIT_OVO -> logos.getValue(paymentMethodType).getValue(isDarkMode)

            else -> error("Unsupported payment method type '$paymentMethodType'")
        }

    private fun getCountryCode() = when (paymentMethodType) {
