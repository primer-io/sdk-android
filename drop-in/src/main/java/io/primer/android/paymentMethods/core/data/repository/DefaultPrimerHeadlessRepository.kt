...
internal class DefaultPrimerHeadlessRepository(
    ...
    override val events: Flow<PrimerEvent> = merge(
        externalEvents,
        callbackFlow {
                ...
                object : PrimerHeadlessUniversalCheckoutListener {
                    ...
                    override fun onFailed(error: PrimerError, checkoutData: PrimerCheckoutData?) {
                        Primer.current.listener?.onFailed(
                            ...
                            errorHandler = object : PrimerErrorDecisionHandler {
                    ...
                    override fun onCheckoutResume(
                        ...
                        Primer.current.listener?.onResumeSuccess(resumeToken, createDecisionHandler(decisionHandler))
                    }
                    ...
                    fun handleFailureImpl(message: String?, isCancellation: Boolean = false) {
                        if (config.settings.uiOptions.isSuccessScreenEnabled) {
                            val errorType = when (config.intent.paymentMethodIntent) {
                                PrimerSessionIntent.CHECKOUT -> if (isCancellation) {
