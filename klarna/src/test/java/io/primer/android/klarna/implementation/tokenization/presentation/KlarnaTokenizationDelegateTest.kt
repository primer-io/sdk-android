...
internal class KlarnaTokenizationDelegateTest {
    ...
    @Test
    fun `mapTokenizationData should return KlarnaVaultPaymentInstrumentParams when session intent is VAULT`() = runBlocking {
        // Given
        val input = KlarnaTokenizationInputable(
        ...
        val customerTokenData = CreateCustomerTokenDataResponse(
        ...
        val expectedParams = KlarnaVaultPaymentInstrumentParams(
    ...
    @Test
    fun `mapTokenizationData should return KlarnaCheckoutPaymentInstrumentParams when session intent is CHECKOUT`() = runBlocking {
        // Given
        val input = KlarnaTokenizationInputable(
            sessionId = "session-id",
            authorizationToken = "auth-token",
        ...
        val customerTokenData = FinalizeKlarnaSessionDataResponse(
            sessionData = klarnaSessionData
        )
        ...
        assert(result.isSuccess)
        val tokenizationParams = result.getOrThrow()
        val expectedParams = KlarnaCheckoutPaymentInstrumentParams(
