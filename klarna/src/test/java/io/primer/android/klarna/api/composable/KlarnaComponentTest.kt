...
class KlarnaComponentTest {
    ...
    @Test
    fun `start() should log event, emit error and call error handler when session creation delegate fails and in HEADLESS mode`() = runTest {
        initComponent(isMockedFlow = false)
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
    ...
    @Test
    fun `start() should log event, log, emit error, and call error handler when session creation delegate fails and in DROP-in mode`() = runTest {
        initComponent(isMockedFlow = false)
    ...
    @Test
    fun `submit() should start authorization without auto finalization, log event, tokenize, handle payment token and emit authorized step when payment is approved and integration type is HEADLESS`() = runTest {
        initComponent(isMockedFlow = false)
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
        mockkObject(KlarnaPaymentCategoryValidator)
    ...
    @Test
    fun `submit() should log event, tokenize, handle token data and emit authorized step when in mocked flow and integration type is HEADLESS`() = runTest {
    ...
    @Test
    fun `submit() should start authorization with auto finalization, log event, tokenize, handle token data and emit authorized step when payment is approved when integration type is DROP-IN`() = runTest {
        initComponent(isMockedFlow = false)
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.DROP_IN
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
        mockkObject(KlarnaPaymentCategoryValidator)
    ...
    @Test
    fun `submit() should start authorization, log event, emit error, and call error handler when payment is authorized but tokenization delegate fails`() = runTest {
        initComponent(isMockedFlow = false)
        coEvery { klarnaPaymentDelegate.handleError(any()) } just Runs
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
        ...
        val exception = Exception()
        coEvery { klarnaTokenizationDelegate.tokenize(any()) } returns Result.failure(
            exception
        )
        val paymentCategory = mockk<KlarnaPaymentCategory> {
            every { identifier } returns "identifier"
    ...
    @Test
    fun `submit() should start authorization, log event, tokenize, handle payment token and emit finalized step when payment is finalized`() = runTest {
        initComponent(isMockedFlow = false)
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
        mockkObject(KlarnaPaymentCategoryValidator)
        ...
        val paymentMethodData = mockk<PrimerPaymentMethodTokenData>(relaxed = true)
        coEvery { klarnaTokenizationDelegate.tokenize(any()) } returns Result.success(
            paymentMethodData
        )
        val paymentDecision = mockk<PaymentDecision>(relaxed = true)
        coEvery {
