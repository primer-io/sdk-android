...
class KlarnaSessionCreationDelegateTest {
    ...
    @Test
    fun `createSession() should return session emitted by session interactor when called and SURCHARGE fee exists and integration type is HEADLESS`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
            coEvery { actionInteractor.invoke(any<MultipleActionUpdateParams>()) } returns
                Result.success(
                    mockk<ClientSessionData> {
                        every { clientSession.fees } returns
                            listOf(
    ...
    @Test
    fun `createSession() should return session emitted by session interactor when called and SURCHARGE fee exists and integration type is DROP_IN`() =
        runTest {
            ...
            coEvery { configurationInteractor.invoke(any()) } returns
                Result.success(
                    mockk<Configuration> {
                        every { clientSession.clientSessionDataResponse.order?.toFees() } returns
                            listOf(
                                PrimerFee(type = "SURCHARGE", amount = 140),
                            )
                    },
    ...
    @Test
    fun `createSession() should return session emitted by session interactor when called and SURCHARGE fee doesn't exist and integration type is HEADLESS`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
            coEvery { actionInteractor.invoke(any<MultipleActionUpdateParams>()) } returns
                Result.success(
    ...
    @Test
    fun `createSession() should return session emitted by session interactor when called and SURCHARGE fee doesn't exist and integration type is DROP_IN`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.DROP_IN
            coEvery { configurationInteractor.invoke(any()) } returns
                Result.success(
    ...
    @Test
    fun `createSession() should return error emitted by session interactor when called and integration type is HEADLESS`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
            coEvery { actionInteractor.invoke(any<MultipleActionUpdateParams>()) } returns
                Result.success(
    ...
    @Test
    fun `createSession() should return error emitted by session interactor when called and integration type is DROP_IN`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.DROP_IN
            coEvery { configurationInteractor.invoke(any()) } returns
                Result.success(
    ...
    @Test
    fun `createSession() should return error emitted by action interactor when called and integration type is HEADLESS`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
            val exception = Exception()
    ...
    @Test
    fun `createSession() should return error emitted by action interactor when called and integration type is DROP_IN`() =
        runTest {
            every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.DROP_IN
            val exception = Exception()
