...
class DefaultBanksComponentTest {
    ...
    @Test
    fun `submit() should log event, startPaymentFlow and call onFinished() when BankWebRedirectComposer succeeds`() = runTest {
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
    ...
    @Test
    fun `submit() should log event and error when BankWebRedirectComposer fails`() = runTest {
        val exception = Exception()
        val primerError = mockk<PrimerError>()
        coEvery { eventLoggingDelegate.logSdkAnalyticsEvent(any(), any()) } just Runs
        coEvery { errorLoggingDelegate.logSdkAnalyticsErrors(any()) } just Awaits
