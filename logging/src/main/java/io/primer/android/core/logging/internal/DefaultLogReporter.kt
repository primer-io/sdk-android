...
interface LogReporter {
     ...
     */
    fun error(message: String, component: String? = null, throwable: Throwable? = null)
}
...
class DefaultLogReporter : LogReporter {
    ...
    override fun info(message: String, component: String?) = log(
        PrimerLog.Info(
            PrimerLogLevel.INFO,
            listOfNotNull(component, message).joinToString(LOG_SEPARATOR)
        )
    ...
    override fun error(message: String, component: String?, throwable: Throwable?) =
        log(
            PrimerLog.Error(
