...
class IPay88ConfigurationDataRepositoryTest {
    ...
    @Test
    fun `getPaymentMethodConfiguration should return WebRedirectConfig when payment method type is found`() = runBlocking {
        // Arrange
        val paymentMethodType = "TEST_PAYMENT_METHOD"
        val paymentMethodConfigId = "12345"
        val locale = Locale.ENGLISH

        val mockPaymentMethod = mockk<PaymentMethodConfigDataResponse> {
    ...
    @Test
    fun `getPaymentMethodConfiguration should throw exception when payment method type is not found`(): Unit = runBlocking {
        // Arrange
        val paymentMethodType = "TEST_PAYMENT_METHOD"
        val params = IPay88ConfigParams(paymentMethodType)
        val configData = mockk<ConfigurationData>(relaxed = true)
