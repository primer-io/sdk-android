...
class StripeAchUserDetailsComponentTest {
    ...
    @Test
    fun `submit() should log event, patch client session, tokenize, call success handler and emit UserDetailsCollected when patching delegate calls succeed and in AUTO mode`() = runTest {
        initComponent()
        ...
        coEvery { stripeAchPaymentDelegate.handlePaymentMethodToken(any(), any()) } returns Result.success(
            PaymentDecision.Pending("", null)
        )
        val stripeAchDecision = StripeAchDecision(
        ...
        coEvery { stripeAchBankFlowDelegate.handle(any(), any(), any()) } returns Result.success(
            StripeAchBankFlowDelegate.StripeAchBankFlowResult(payment = payment, mandateTimestamp = "timestamp")
        )
    ...
    @Test
    fun `submit() should log event, patch client session, tokenize, call pending resume handler, manual success handler and emit UserDetailsCollected when patching delegate calls succeed and in MANUAL mode`() = runTest {
        initComponent()
    ...
    @Test
    fun `submit() should log event, patch client session, emit UserDetailsCollected, and emit error if tokenization delegate call fails`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
        val error = Exception()
        coEvery {
    ...
    @Test
    fun `submit() should log event, patch client session, emit UserDetailsCollected, tokenize and emit error if payment delegate call fails`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
        val error = Exception()
        coEvery {
    ...
    @Test
    fun `submit() should log event, patch client session, emit UserDetailsCollected, tokenize, create payment, call error handler and emit error if bank flow delegate call fails with non-cancellation error`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
        val error = Exception()
        coEvery {
    ...
    @Test
    fun `submit() should log event, patch client session, emit UserDetailsCollected, tokenize, create payment, call error handler and emit error if bank flow delegate call fails with PaymentMethodCancelledException`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
    ...
    @Test
    fun `submit() should log event, patch client session, emit UserDetailsCollected, tokenize, create payment and swallow error if bank flow delegate call fails with CancellationException`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
    ...
    @Test
    fun `submit() should log event, not tokenize, not create payment and call error handler emit error if client session patching fails`() = runTest {
        every { primerSettings.sdkIntegrationType } returns SdkIntegrationType.HEADLESS
        initComponent()
        val error = Exception()
        coEvery {
