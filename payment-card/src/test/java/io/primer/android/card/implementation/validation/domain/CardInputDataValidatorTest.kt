...
internal class CardInputDataValidatorTest {
    ...
    @Test
    fun `validate should return errors when card number, expiry date, and cvv are invalid`() =
        runTest {
            ...
            mockkConstructor(CardCvvValidator::class) {
                coEvery { anyConstructed<CardCvvValidator>().validate(any()) } returns
                    PrimerInputValidationError(
                        "invalid-cvv",
                        "CVV is not valid.",
            ...
            coEvery { CardExpiryDateValidator().validate(any()) } returns
                PrimerInputValidationError(
                    "invalid-expiry-date",
    ...
    @Test
    fun `validate should return no errors when all inputs are valid`() =
        runTest {
    ...
    @Test
    fun `validate should not validate cardholder name when it is disabled`() =
        runTest {
            val cardData = PrimerCardData("4111111111111111", "12/2025", "123", "John Doe")
    ...
    @Test
    fun `validate should return errors for invalid cardholder name when it is enabled`() =
        runTest {
            ...
            coEvery { CardExpiryDateValidator().validate(any()) } returns null
            coEvery { CardholderNameValidator().validate(any()) } returns
                PrimerInputValidationError(
