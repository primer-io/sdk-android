...
class RemoteVaultedPaymentMethodDeleteDataSourceTest {
    ...
    fun `request is made with correct endpoint, method and headers`(apiVersion: PrimerApiVersion): Unit =
        runTest {
            ...
            val tested = RemoteVaultedPaymentMethodDeleteDataSource(primerHttpClient) { apiVersion }
            ...
            assertEquals("https://example.com/payment-instruments/dataRequest/vault", request.url.toString())
    ...
    fun `response is processed when the server responds in time`() =
        runTest {
            ...
            val tested =
                RemoteVaultedPaymentMethodDeleteDataSource(
                    PrimerHttpClient(
