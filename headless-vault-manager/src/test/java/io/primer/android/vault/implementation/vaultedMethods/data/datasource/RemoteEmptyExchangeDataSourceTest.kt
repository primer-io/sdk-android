...
class RemoteEmptyExchangeDataSourceTest {
    private val dataRequest = mockk<EmptyExchangeDataRequest>()

    private val input =
        mockk<BaseRemoteUrlRequest<EmptyExchangeDataRequest>> {
    ...
    fun `request is made with correct endpoint, method, request body and headers`(apiVersion: PrimerApiVersion): Unit =
        runTest {
            ...
            val tested = RemoteEmptyExchangeDataSource(primerHttpClient) { apiVersion }
