...
internal class ConfigurationDataRepositoryTest {
    ...
    @Test
    fun `fetchConfiguration with ForceCache should serve configuration from local cache`() =
        runTest {
    ...
    @Test
    fun `fetchConfiguration with ForceNetwork should call remote API and should not get configuration from cache`() =
        runTest {
            ...
            every { configurationResponse.headers } returns
                mapOf(
                    PrimerSessionConstants.PRIMER_SESSION_CACHE_TTL_HEADER to
                        listOf(
    ...
    @Test
    fun `fetchConfiguration with CacheFirst and valid cache should serve configuration from global cache`() =
        runTest {
            val configurationData = mockk<ConfigurationData>(relaxed = true)
            val configurationDataResponse = mockk<ConfigurationDataResponse>(relaxed = true)
            val configurationResponse = mockk<PrimerResponse<ConfigurationDataResponse>>(relaxed = true)
            every { configurationResponse.body } returns configurationDataResponse
            val configuration = mockk<Configuration>(relaxed = true)
            every { globalConfigurationCache.get() } returns
                mockk {
                    every { first } returns
                        mockk {
    ...
    @Test
    fun `fetchConfiguration with CacheFirst and invalid cache should call remote API and should not get configuration from cache`() =
        runTest {
            val configurationData = mockk<ConfigurationData>(relaxed = true)
            val configurationDataResponse = mockk<ConfigurationDataResponse>(relaxed = true)
            val configurationResponse = mockk<PrimerResponse<ConfigurationDataResponse>>(relaxed = true)
