        ...
        when (implementationType) {
            PaymentMethodImplementationType.NATIVE_SDK -> {
                when (PaymentMethodType.safeValueOf(type)) {
                    ...
                    PaymentMethodType.XENDIT_OVO -> PhoneNumberFactory(paymentMethodType = type).build()
                    ...
                    PaymentMethodType.OMISE_PROMPTPAY -> QrCodeFactory(paymentMethodType = type).build()
