...
internal class DefaultCheckoutSuccessHandler(
    ...
    private val _payments = MutableSharedFlow<Payment>()

    override val checkoutCompleted: Flow<Payment> = _payments

    override suspend fun handle(payment: Payment, additionalInfo: PrimerCheckoutAdditionalInfo?) {
        analyticsRepository.addEvent(
            SdkFunctionParams(
                HeadlessUniversalCheckoutAnalyticsConstants.ON_CHECKOUT_COMPLETED
            )
        )
        _payments.emit(payment)
