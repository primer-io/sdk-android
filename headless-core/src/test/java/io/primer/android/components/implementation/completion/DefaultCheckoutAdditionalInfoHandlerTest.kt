...
class DefaultCheckoutAdditionalInfoHandlerTest {
    ...
    @Test
    fun `handle() calls onResumePending when completesCheckout is true and paymentHandling is MANUAL`() =
        runTest {
    ...
    @Test
    fun `handle() calls onCheckoutCompleted when completesCheckout is true and paymentHandling is AUTO`() =
        runTest {
            Dispatchers.setMain(StandardTestDispatcher(testScheduler))
    ...
    @Test
    fun `handle() calls onCheckoutAdditionalInfoReceived when completesCheckout is false`() =
        runTest {
            Dispatchers.setMain(StandardTestDispatcher(testScheduler))
