...
abstract class TokenizationRequestV2<T : BasePaymentInstrumentDataRequest> :
    ...
    companion object {
        @JvmField
        val serializer =
            JSONObjectSerializer<TokenizationRequestV2<in BasePaymentInstrumentDataRequest>> { t ->
                when (t) {
...
data class TokenizationVaultRequestV2<T : BasePaymentInstrumentDataRequest>(
    ...
    companion object {
        ...
        @JvmField
        val serializer =
            JSONObjectSerializer<TokenizationVaultRequestV2<BasePaymentInstrumentDataRequest>> { t ->
                JSONObject().apply {
                    put(
...
data class TokenizationCheckoutRequestV2<T : BasePaymentInstrumentDataRequest>(
    ...
    companion object {
        ...
        @JvmField
        val serializer =
            JSONObjectSerializer<TokenizationCheckoutRequestV2<BasePaymentInstrumentDataRequest>> { t ->
                JSONObject().apply {
                    put(
                        PAYMENT_INSTRUMENT_FIELD,
                        t.paymentInstrumentSerializer.serialize(t.paymentInstrument),
