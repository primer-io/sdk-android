...
class DefaultPaymentMethodTokenHandlerTest {
    ...
    @Test
    fun `handle should use AUTO strategy by default`() = runTest {
        // Arrange
        ...
        coEvery { createPaymentInteractor.invoke(CreatePaymentParams("testToken")) } returns Result.success(
            paymentDecision
        )
    ...
    @Test
    fun `handle should use MANUAL strategy when configured`() = runTest {
        // Arrange
        val paymentMethodTokenData = mockk<PrimerPaymentMethodTokenData>(relaxed = true)
        val primerSessionIntent = PrimerSessionIntent.CHECKOUT
        val paymentDecision = mockk<PaymentDecision>()
    ...
    @Test
    fun `handle should use VAULT strategy when intent is VAULT`() = runTest {
        // Arrange
        val paymentMethodTokenData = mockk<PrimerPaymentMethodTokenData>(relaxed = true)
