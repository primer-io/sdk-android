...
class DefaultPaymentResumeHandlerTest {
    ...
    private val postResumeHandler = mockk<PostResumeHandler>(relaxed = true) {
    ...
    @Test
    fun `handle should use AUTO strategy when payment handling is auto`() = runTest {
        // Arrange
...
//        assertEquals(paymentDecision, result.getOrNull())
    ...
    @Test
    fun `handle should use MANUAL strategy when payment handling is manual`() = runTest {
        // Arrange
        val resumeToken = "resumeToken"
