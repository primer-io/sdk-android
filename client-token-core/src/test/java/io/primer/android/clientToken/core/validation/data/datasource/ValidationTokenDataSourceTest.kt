...
class ValidationTokenDataSourceTest {
    ...
    private val input =
        mockk<BaseRemoteHostRequest<ValidationTokenDataRequest>> {
    ...
    fun `request is made with correct endpoint, method, request body and headers`(apiVersion: PrimerApiVersion): Unit =
        runTest {
            ...
            val tested = ValidationTokenDataSource(primerHttpClient) { apiVersion }
            ...
            assertEquals("https://example.com/client-token/validate", request.url.toString())
            val expectedResult =
