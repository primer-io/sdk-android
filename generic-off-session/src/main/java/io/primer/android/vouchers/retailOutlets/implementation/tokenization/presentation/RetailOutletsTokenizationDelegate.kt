...
internal class RetailOutletsTokenizationDelegate(
    ...
    TokenizationCollectedDataMapper<RetailOutletsTokenizationInputable,
        RetailOutletsPaymentInstrumentParams> {

    override suspend fun mapTokenizationData(input: RetailOutletsTokenizationInputable):
        Result<TokenizationParams<RetailOutletsPaymentInstrumentParams>> = configurationInteractor(
        RetailOutletsConfigParams(paymentMethodType = input.paymentMethodType)
    ).map { configuration ->
        TokenizationParams(
            paymentInstrumentParams = RetailOutletsPaymentInstrumentParams(
