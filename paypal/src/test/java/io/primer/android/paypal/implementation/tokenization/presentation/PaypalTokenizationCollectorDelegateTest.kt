...
internal class PaypalTokenizationCollectorDelegateTest {
    ...
    @BeforeEach
    fun setUp() {
        delegate = PaypalTokenizationCollectorDelegate(
    ...
    @Test
    fun `startDataCollection should navigate to correct URL for PaypalCheckoutConfiguration`() = runTest {
        // Given
        val params = PaypalTokenizationCollectorParams(primerSessionIntent = mockk())
        val configuration = PaypalConfig.PaypalCheckoutConfiguration(
        ...
        coEvery { configurationInteractor(PaypalConfigParams(params.primerSessionIntent)) } returns Result.success(
            configuration
        )
    ...
    @Test
    fun `startDataCollection should navigate to correct URL for PaypalVaultConfiguration`() = runTest {
        // Given
        val params = PaypalTokenizationCollectorParams(primerSessionIntent = mockk())
        val configuration = PaypalConfig.PaypalVaultConfiguration(
            paymentMethodConfigId = "config-id",
            successUrl = "https://success.url",
            cancelUrl = "https://cancel.url"
        )
        val agreementResponse = PaypalBillingAgreement(
            approvalUrl = "https://approval.url",
            successUrl = "https://success.url",
            paymentMethodConfigId = "config-id",
            cancelUrl = "https://cancel.url"
    ...
    @Test
    fun `startDataCollection should handle errors`() = runTest {
        // Given
        val params = PaypalTokenizationCollectorParams(primerSessionIntent = mockk())
        coEvery { configurationInteractor(PaypalConfigParams(params.primerSessionIntent)) } returns Result.failure(
