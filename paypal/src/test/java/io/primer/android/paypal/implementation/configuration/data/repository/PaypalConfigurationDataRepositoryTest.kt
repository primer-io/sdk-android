...
class PaypalConfigurationDataRepositoryTest {
    ...
    @Test
    fun `getPaymentMethodConfiguration should return PaypalCheckoutConfiguration`() = runTest {
        // Arrange
        val orderId = "order123"
        val currencyCode = "USD"
        val paymentMethodConfigId = "paypalConfigId"

        val orderDataResponse = OrderDataResponse(
            orderId = orderId,
            currencyCode = currencyCode,
            totalOrderAmount = 100
        )
        val paymentMethodConfigDataResponse = mockk<PaymentMethodConfigDataResponse> {
            every { type } returns PaymentMethodType.PAYPAL.name
            every { id } returns paymentMethodConfigId
        }
        val config = mockk<ConfigurationData> {
            every { clientSession } returns mockk {
        ...
        val expectedConfig = PaypalConfig.PaypalCheckoutConfiguration(
    ...
    @Test
    fun `getPaymentMethodConfiguration should return PaypalVaultConfiguration`() = runTest {
        // Arrange
        val paymentMethodConfigId = "paypalConfigId"
        ...
        val expectedConfig = PaypalConfig.PaypalVaultConfiguration(
            paymentMethodConfigId = paymentMethodConfigId,
            successUrl = expectedSuccessUrl,
            cancelUrl = expectedCancelUrl
        )
