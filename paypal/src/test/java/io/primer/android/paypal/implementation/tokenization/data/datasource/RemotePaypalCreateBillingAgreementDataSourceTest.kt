...
class RemotePaypalCreateBillingAgreementDataSourceTest {
    @Test
    fun `response is processed when the server responds in time`() =
        runTest {
            ...
            val input =
                mockk<BaseRemoteHostRequest<PaypalCreateBillingAgreementDataRequest>> {
                    every { host } returns mockWebServer.url("/").toString()
                    every { data } returns mockk(relaxed = true)
                }

            val tested =
                RemotePaypalCreateBillingAgreementDataSource(
                    PrimerHttpClient(
