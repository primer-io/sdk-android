...
class RetryMechanismKtTest {
    ...
    @RelaxedMockK
    internal lateinit var logger: LogReporter

    @RelaxedMockK
    internal lateinit var response: Response

    @RelaxedMockK
    internal lateinit var messagePropertiesEventProvider: MessagePropertiesEventProvider
    ...
    fun `logger should log network error`() = runTest {
        ...
        verify { logger.warn(match { it.contains("Network error encountered") }) }
    }
    ...
    fun `logger should log server error`() = runTest {
        ...
        verify { logger.warn(match { it.contains("HTTP 500 error encountered") }) }
    }
