...
internal class CardCvvValidatorTest {
    ...
    @Test
    fun `validate should not return error when CVV is valid and locally resolved card is VISA`() = runTest {
    ...
    @Test
    fun `validate should not return error when CVV is valid and remotely resolved card is VISA`() = runTest {
    ...
    @Test
    fun `validate should not return error when CVV is valid and remotely resolved card is CARTES_BANCAIRES`() = runTest {
        val primerCardNumberEntryMetadata = mockk<PrimerCardNumberEntryMetadata>(relaxed = true)
        val primerCardNetworksMetadata = mockk<PrimerCardNetworksMetadata>(relaxed = true)
        val primerCardNetwork = mockk<PrimerCardNetwork>(relaxed = true)
    ...
    @Test
    fun `validate should return error 'invalid-cvv' when CVV data is null`() = runTest {
        val resultError = validator.run {
            validate(null)
        }
    ...
    @Test
    fun `validate should return error 'invalid-cvv' when CVV is blank`() = runTest {
        val resultError = validator.run {
    ...
    @Test
    fun `validate should return error 'invalid-cvv' when CVV size is not correct`() = runTest {
        val resultError = validator.run {
    ...
    @Test
    fun `validate should return error 'invalid-cvv' when CVV has incorrect characters`() = runTest {
        val resultError = validator.run {
