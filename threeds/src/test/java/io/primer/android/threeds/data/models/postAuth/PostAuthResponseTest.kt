...
class PostAuthResponseTest {

    @Test
    fun `deserializer should deserialize PostAuthResponse from JSONObject correctly`() {
        ...
        val authenticationJson = JSONObject().apply {
        ...
        val postAuthResponse = PostAuthResponse.deserializer.deserialize(responseJson)

        // Assert the deserialized object matches the expected values
        assertEquals(token, postAuthResponse.token)
        assertEquals(authentication, postAuthResponse.authentication)
        assertEquals(resumeTokenValue, postAuthResponse.resumeToken)
    }
}
