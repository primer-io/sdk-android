...
internal class ThreeDsErrorMapperTest {
    ...
    @Test
    fun `getPrimerError(ThreeDsInvalidStatusException) should return ThreeDsError_ThreeDsChallengeInvalidStatusError`() {
        val exception = mockk<ThreeDsInvalidStatusException>(relaxed = true)
        runTest {
            val error = mapper.getPrimerError(exception)
            assertEquals<Class<*>>(
    ...
    @Test
    fun `getPrimerError(Exception) should throw IllegalStateException`() {
        runTest {
